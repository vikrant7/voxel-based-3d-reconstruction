...
data #point cloud data imported
data = np.array(data) # converting the array to numpy array
voxel_size #taken input from the user
L # largest size of hole to be filled -- taken input from the user
...
for i in range(len(data)):
	voxel_idx = int((data[i][0]-min(data[:, 0]))/voxel_size) + 2  # From 2(0) to nx+1(nx-1)
	voxel_idy = int((data[i][1]-min(data[:, 1]))/voxel_size) + 2  # From 2(0) to ny+1(ny-1)
	voxel_idz = int((data[i][2]-min(data[:, 2]))/voxel_size) + 2  # From 2(0) to nz+1(nz-1)
...
for d in range(1, L+1):
	...
	while(i<len(b_voxel_index)):
		...
		nfc3 = NFC3(pg_voxel[pg_voxel[x-1:x+2, y-1:y+2, z-1:z+2]]) # passing the neighbourhood of 3x3
		nbc5 = NBC5(pg_voxel[pg_voxel[x-2:x+3, y-2:y+3, z-2:z+3]]) # passing the neighbourhood of 5x5
		if(nfc3>1 or nfc3==0):
	...
	for i in range(len(b_voxel_index)):
		for j in range(len(i+1, len(b_voxel_index))):
